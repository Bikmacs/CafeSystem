...
public class UserControllerTests: BaseIntegrationTest
{
    ...
    public async Task RegisterUser_AsAdmin_Success()
    {
        ...
        Assert.That(response.IsSuccessStatusCode, Is.True, $"Сервер не смог зарегистрировать пользователя. Ошибка: {responseString}");
        ...
        Assert.That(userInDb.PasswordHash, Is.Not.EqualTo(_user.Password), "ОПАСТНОСТЬ: Пароль сохранен в открытом виде без хэширования");
    }
